...
class AlcadaViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun selectDataset(id: String) { if (datasets.value.any { it.id == id }) _selectedDataset.value = id else _importState.value = OperationState(error = "O conjunto de dados selecionado não está mais disponível.") }
    ...
    fun runResearch(options: ResearchOptions = ResearchOptions()) {
        ...
        if (options.intensive && health.thermalStatus >= PowerManager.THERMAL_STATUS_MODERATE) return failResearch("A temperatura do aparelho já está elevada. Aguarde o resfriamento ou use o modo normal.")
