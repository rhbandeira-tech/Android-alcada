...
private fun DatasetSelector(values: List<DatasetEntity>, selected: String?, select: (String) -> Unit) {
    Card {
        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (values.isEmpty()) {
                Text("Importe um CSV em Mais › Dados", color = Pending)
            } else {
...
private fun Metrics(metrics: BacktestMetrics) {
    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        Metric(pct(metrics.winRate), "win rate", Analytic, Modifier.weight(1f))
        Metric("%.2f".format(metrics.netProfit), "resultado", if (metrics.netProfit >= 0) Positive else Negative, Modifier.weight(1f))
        Metric("%.2f".format(metrics.maxDrawdown), "drawdown", Negative, Modifier.weight(1f))
    }
}
...
private fun ResultCharts(result: BacktestResult, analysis: QuantAnalysis?) {
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        LineChart("Equity / lucro acumulado", result.equity, Positive)
        LineChart("Drawdown", result.drawdown, Negative)
        LineChart("Rolling win rate", result.rollingWinRate, Analytic)
        BarChart("Distribuição dos trades", ChartAnalytics.pnlDistribution(result.trades))
        BarChart("Desempenho por hora (UTC)", ChartAnalytics.performanceByHour(result.trades))
        BarChart("Desempenho por dia", ChartAnalytics.performanceByDay(result.trades))
        analysis?.let {
            BarChart("Heatmap dia × hora", it.heatmap)
            BarChart("In-sample × out-of-sample", it.isOos)
            BarChart("Timeframe × expiração", it.timeframeExpiration)
    ...
    Card {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(14.dp)) {
                Text("IS ${pct(data.getOrNull(1))}")
                Text("OOS ${pct(data.getOrNull(2))}")
                Text("PF $profitFactor")
            }
            Text(
...
private fun HistoryBacktest(backtest: BacktestEntity) {
    ...
    ListItem(
        headlineContent = { Text("Backtest ${backtest.market}") },
        supportingContent = { Text("${date(backtest.createdAt)} • ${data.getOrNull(0) ?: 0} trades • resultado $result") },
