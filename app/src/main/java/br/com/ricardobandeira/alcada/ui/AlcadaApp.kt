...
private fun scriptRule(encoded: String, language: String, direction: String? = null): String? {
    ...
    val expression = when (feature) {
        ...
        "candleSequenceAbs" -> "math.abs(close>open ? (close[1]>open[1] ? (close[2]>open[2] ? (close[3]>open[3] ? 4.0 : 3.0) : 2.0) : 1.0) : (close<open ? (close[1]<open[1] ? (close[2]<open[2] ? (close[3]<open[3] ? -4.0 : -3.0) : -2.0) : -1.0) : 0.0))"
